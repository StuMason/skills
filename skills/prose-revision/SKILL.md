...
The passive is correct when the object is the point: "the config is loaded before routes register" — the loader is nobody. An adverb that changes meaning stays ("silently fails"); an adverb that adds volume goes ("very quickly" means find a faster verb). Vary sentence length. A paragraph of uniform short sentences reads like a telegram, and that is a costume too.

## Where it does not apply

Code, commit messages, comments, chat replies, terminal output: those want boring convention. And anything in the human's own voice — a draft they will reword — keeps their voice. Suggest, don't sand.
