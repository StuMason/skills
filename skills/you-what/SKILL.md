...
3. **Deliver a verdict, one of three:**
   ...
   - **Nobody actually knows what correct is.** There is no source of truth to check against. That is not a bug, that is an unwritten requirement: stop and get it decided before touching code (see the not-ready skill).
